...
map_supported_devices() {
	...
	for pci_ids_header in \
		"$rootdir/include/spdk/pci_ids.h" \
		"$rootdir/third-party/spdk/include/spdk/pci_ids.h"; do
		[[ -e $pci_ids_header ]] && break
	done
	[[ -e $pci_ids_header ]] || return 1
	...
	done < <(grep -E "$ids" "$pci_ids_header")
