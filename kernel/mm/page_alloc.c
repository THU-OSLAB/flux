			...
			struct page **page_array)
{
	...
	if (gfp & __GFP_UBUF) {
		zone = &NODE_DATA(page_to_nid(page))->node_zones[gfp_zone(gfp)];
	} else {
							...
							nodemask_t *nodemask)
{
	...
	if (WARN_ON_ONCE_GFP(order > MAX_ORDER, gfp)) {
		pr_err("order %u is out of bounds\n", order);
		return NULL;
	}
...
static void zone_set_pageset_high_and_batch(struct zone *zone, int cpu_online)
{
	...
	pr_info("  %s zone: %lu pages batch=%x high=%x\n", zone->name,
		zone->present_pages, new_batch, new_high);
