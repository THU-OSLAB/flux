...
#define BFLUX_CRCERR_ABORT		0x0800
#define BFLUX_CD_POL_HIGH		0x1000
#define BFLUX_GPI_CD			0x2000
#define BFLUX_DATA3_CD			0x4000
#define BFLUX_INT_ENABLE			0x8000
...
static void wmt_reset_hardware(struct mmc_host *mmc)
{
	...
	writew(BFLUX_INT_ENABLE | BFLUX_GPI_CD, priv->sdmmc_base + SDMMC_BLKLEN);
...
static int wmt_mci_resume(struct device *dev)
{
	...
	if (mmc) {
		...
		reg_tmp = readw(priv->sdmmc_base + SDMMC_BLKLEN);
		writew(reg_tmp | (BFLUX_GPI_CD | BFLUX_INT_ENABLE),
		       priv->sdmmc_base + SDMMC_BLKLEN);
