#ifndef _FLUX_IOK_CSUM_H
#define _FLUX_IOK_CSUM_H
 ...
 * All rights reserved.
 *
 * These checksum routines were imported from the original rte checksum helpers.
 */
				       ...
				       uint32_t daddr, uint16_t l4len)
{
	...
	psd_hdr.saddr = cpu_to_be32(saddr);
	psd_hdr.daddr = cpu_to_be32(daddr);
	psd_hdr.zero = 0;
	psd_hdr.proto = proto;
	psd_hdr.len = cpu_to_be16(l4len);
...
static inline uint16_t ipv4_cksum(const struct iphdr *iphdr)
{
	uint16_t cksum;
