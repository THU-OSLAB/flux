...
static const struct flux_iokd_cfg_entry *flux_iokd_find_entry(const char *json,
							      const jsmntok_t *tok)
{
	static const struct flux_iokd_cfg_entry entries[] = {
		...
		{ "nic_pci_addr", IOKD_CFG_STRING, &flux_iokd_cfg.nic_pci_addr },
	...
	for (pos = 1; pos < ret; pos++) {
		...
		case IOKD_CFG_STRING:
			if (val->type != JSMN_STRING)
				return -1;
			if (flux_iokd_copy_string(entry->dst, json + val->start,
...
int flux_iokd_config_load(int argc, char **argv)
{
	...
	while ((opt = getopt_long(argc, argv, "c:h", long_opts, NULL)) != -1) {
		switch (opt) {
		case 'c':
			if (flux_iokd_copy_string(&flux_iokd_cfg.config_path, optarg,
						  strlen(optarg)) < 0)
				return -1;
			break;
		case 'h':
	...
	while ((opt = getopt_long(argc, argv, "c:h", long_opts, NULL)) != -1) {
		...
		case 'h':
			break;
		case 1:
			if (flux_iokd_copy_string(&flux_iokd_cfg.cpu_range, optarg,
						  strlen(optarg)) < 0)
				return -1;
			break;
		case 2:
			if (flux_iokd_copy_string(&flux_iokd_cfg.sock_path, optarg,
						  strlen(optarg)) < 0)
				return -1;
			break;
		case 3:
			if (flux_iokd_copy_string(&flux_iokd_cfg.nic_pci_addr, optarg,
						  strlen(optarg)) < 0)
				return -1;
			break;
		case 4:
