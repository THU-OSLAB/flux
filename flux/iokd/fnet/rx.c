...
void flux_iokd_rx_client_queue_unregister(struct flux_iokd_client *client)
{
	...
	do {
		head = atomic_load_relaxed(
			&flux_iokd.pending_unregister_clients);
...
void flux_iokd_rx_process_pending_clients(void)
{
	...
	client = __atomic_exchange_n(&flux_iokd.pending_unregister_clients,
				     NULL, __ATOMIC_ACQUIRE);
	while (client) {
			       ...
			       struct rte_mbuf *buf)
{
	...
	__atomic_add_fetch(&client->rx_packets, 1, __ATOMIC_RELAXED);
	__atomic_add_fetch(&client->rx_bytes, rte_pktmbuf_pkt_len(buf),
			   __ATOMIC_RELAXED);
