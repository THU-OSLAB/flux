...
static unsigned int flux_iokd_tx_next_cpu_rr[FLUX_IOKD_MAX_CLIENTS];
static unsigned int flux_iokd_tx_next_client_rr;

static bool flux_iokd_tx_quota_available(struct flux_iokd_client *client,
					 uint64_t now)
{
	int64_t quota_us = atomic_load_relaxed(&client->cpu_quota);
	uint64_t period_us = atomic_load_relaxed(&client->cpu_period);
	uint64_t period_ns;

	if (quota_us <= 0 || period_us == 0)
		return true;
	period_ns = period_us * NSEC_PER_USEC;
	if (!client->tx_quota_window_start_ns ||
	    now - client->tx_quota_window_start_ns >= period_ns) {
		client->tx_quota_window_start_ns = now;
		client->tx_quota_used_ns = 0;
	}
	return client->tx_quota_used_ns < (uint64_t)quota_us * NSEC_PER_USEC;
}
				 ...
				 const struct flux_iokd_pkt *pkt)
{
	...
	buf->ol_flags = 0;
	buf->hash.sched.queue_id =
		atomic_load_relaxed(&pkt->client->net_class_id);
	net_priority = atomic_load_relaxed(&pkt->client->net_priority);
	buf->hash.sched.traffic_class =
		(uint8_t)(net_priority > UINT8_MAX ? UINT8_MAX : net_priority);
	buf->hash.sched.color = 0;
...
bool flux_iokd_tx_burst(void)
{
	...
	if (budget > 0) {
		for (i = 0; i < FLUX_IOKD_MAX_CLIENTS && budget > 0; i++) {
			int client_idx =
				(flux_iokd_tx_next_client_rr + (unsigned int)i) %
				FLUX_IOKD_MAX_CLIENTS;
			struct flux_iokd_client *client =
				flux_iokd_client_load(client_idx);
			uint64_t allowance;
			...
			if (!flux_iokd_tx_quota_available(client, service_start))
				continue;
			client->tx_deficit += flux_iokd_client_tx_quantum(client);
			if (client->tx_deficit > 4 * FLUX_FNET_TX_BURST_SIZE)
				client->tx_deficit = 4 * FLUX_FNET_TX_BURST_SIZE;
			allowance = client->tx_deficit < (uint64_t)budget ?
					    client->tx_deficit :
					    (uint64_t)budget;
			for (j = 0; j < client->nr_cpus && allowance > 0; j++) {
				int cpu_idx =
					flux_iokd_tx_next_cpu_rr[client_idx]++ %
					      client->nr_cpus;

				ret = flux_iokd_tx_drain_queue(
					client, cpu_idx, (int)allowance,
					&flux_iokd_tx_pkts[flux_iokd_tx_n_pkts]);
				flux_iokd_tx_n_pkts += ret;
				budget -= ret;
				allowance -= (uint64_t)ret;
				sent += ret;
			}
			if (sent == 0)
				client->tx_deficit = 0;
			else {
				client->tx_deficit -= (uint64_t)sent;
				client->tx_quota_used_ns += now_ns() - service_start;
			}
		}
		flux_iokd_tx_next_client_rr =
			(flux_iokd_tx_next_client_rr + 1) %
			FLUX_IOKD_MAX_CLIENTS;
	}
	...
	if (flux_iokd_tx_n_bufs > 0) {
		...
		for (i = 0; i < ret; i++) {
			struct flux_iokd_tx_priv *priv =
				rte_mbuf_to_priv(flux_iokd_tx_bufs[i]);
			struct flux_iokd_client *client = priv->client;

			if (!client)
				continue;
			__atomic_add_fetch(&client->tx_packets, 1,
					   __ATOMIC_RELAXED);
			__atomic_add_fetch(&client->tx_bytes,
					   rte_pktmbuf_pkt_len(
						   flux_iokd_tx_bufs[i]),
					   __ATOMIC_RELAXED);
		}
