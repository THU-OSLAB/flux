...
static int flux_iokd_build_socket_mem(char *buf, size_t len)
{
	...
	for (i = 0; i <= max_node; i++) {
		int ret = snprintf(buf + used, len - used, "%s%d",
				   i ? "," : "--socket-mem=",
				   i == node ? 128 : 0);
...
static int flux_iokd_eal_init(void)
{
	...
	if (flux_iokd_cfg.no_network) {
		argv[argc++] = "--no-huge";
		argv[argc++] = "--no-pci";
	} else if (flux_iokd_cfg.nic_pci_addr && flux_iokd_cfg.nic_pci_addr[0]) {
...
void flux_iokd_fnet_fini(void)
{
	...
	flux_iokd_fnet_stop();
	if (flux_iokd.fnet_has_port && rte_eth_dev_is_valid_port(FLUX_FNET_PORT))
